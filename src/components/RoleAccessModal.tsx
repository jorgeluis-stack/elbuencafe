...
export const RoleAccessModal: React.FC<RoleAccessModalProps> = ({ isOpen, onClose, onRoleSelected }) => {
    const { users, saveUser } = useOrders(); // Usar usuarios del contexto (cargados desde Supabase)
    ...
    const handleLogin = (e: React.FormEvent) => {
        ...
        setTimeout(async () => {
            ...
            // Admin puede acceder a cualquier sección
            if (userFound.roles.includes('admin')) {
                // Admin tiene acceso total - permitir cualquier rol seleccionado
                setIsLoading(false);
            ...
            // Success - call callback with credentials
            setIsLoading(false);
            onRoleSelected(selectedRole, username, password);
            handleCloseInternal();
        }, 500);
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <motion.div
                ...
                className="bg-brand-crema-light w-full max-w-md rounded-3xl shadow-2xl overflow-hidden border border-brand-gold/20 relative"
            >
                {/* Close button */}
                <button
